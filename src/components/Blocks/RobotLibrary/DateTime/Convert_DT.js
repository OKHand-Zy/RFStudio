...
// DateTime: Convert Time
const convert_time_MutatorMixin = {
  mutationToDom: function() {
    const container = document.createElement('mutation');
    container.setAttribute('hasResultFormat', this.hasResultFormat_);
    container.setAttribute('hasExcludeMillis', this.hasExcludeMillis_);
...
// Main block definition
Blockly.Blocks['rbl_datetime_convert_time'] = {
  init: function() {
    this.containerBlockType = 'gcd_container';
    ...
    this.setOutput(true, null);
    this.setColour(block_color);
    this.setTooltip('DateTime: Convert Time');
    this.setHelpUrl('https://robotframework.org/robotframework/latest/libraries/DateTime.html#Convert%20Time'); 
  },
...
Blockly.Blocks['gcd_container'] = {
  init: function() {
    ...
    this.setColour(block_color);
    this.contextMenu = false;
    this.setTooltip("Converts between supported time formats.");
  }
};
...
pythonGenerator.forBlock['rbl_datetime_convert_time'] = function(block) {
  let code = 'Convert Time';
